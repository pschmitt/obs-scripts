...
cmdtxt.DEFAULT_COMMAND = string.format(
  'timewarrior::is-on && echo -n "on " || echo -n "off "; timewarrior::today-total --minutes || echo "N/A"',
  DEFAULT_VALUE
)

local function update_text_source_with_cmd_output()
  local text =
    utils.exec_cmd(cmdtxt.COMMAND, cmdtxt.TIMEOUT, cmdtxt.SHELL, cmdtxt.DEBUG)
  ...
  local tw_status, tw_text = string.match(text, "(%S+)%s*(.*)")

  local color = "#ffffff"
  local hours = utils.extract_first_digit(tw_text)
  ...
  -- prepend emoji
  local emoji = "⏱️"
  if string.find(tw_status, "on") then
  else
    emoji = "⏸️"
  end
  tw_text = string.format("%s %s", emoji, tw_text)
  utils.update_text_source(cmdtxt.TARGET_SOURCE, tw_text, color)
end
